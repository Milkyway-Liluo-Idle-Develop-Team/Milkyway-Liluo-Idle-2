...
func openFullDB(b *testing.B) *db.DB {
	...
	for _, s := range []string{
		...
		`CREATE TABLE player_discovered_items (user_id INTEGER NOT NULL, item_id INTEGER NOT NULL,
...
func BenchmarkTick_OnDisk(b *testing.B) {
	...
	for _, s := range []string{
		...
		`CREATE TABLE IF NOT EXISTS player_unlocked_events (user_id INTEGER NOT NULL, event_id INTEGER NOT NULL,
			unlocked_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
			PRIMARY KEY (user_id, event_id))`,
		`CREATE TABLE IF NOT EXISTS player_discovered_items (user_id INTEGER NOT NULL, item_id INTEGER NOT NULL,
			discovered_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
			PRIMARY KEY (user_id, item_id))`,
